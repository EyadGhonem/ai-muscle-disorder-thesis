#!/usr/bin/env python3
"""
Extract REAL radiomics features from PatientData.mat using HDF5
Robust feature extraction for different MATLAB versions
"""
...
def load_matlab_data():
    """Load ultrasound images from PatientData.mat with multiple methods"""
    ...
    try:
        # Method 1: Try scipy.io (for older MATLAB versions)
        ...
        print(f"scipy.io failed: {e1}")
        
        try:
            # Method 2: Try h5py (for MATLAB v7.3+)
            print("Trying h5py...")
            with h5py.File(str(mat_path), 'r') as f:
                ...
                if 'images' in f:
                    images = f['images'][:]
                    print(f"Images shape from HDF5: {images.shape}")
                    return {'images': images, 'method': 'h5py'}, 'h5py'
                else:
                    print("Available keys:", keys)
                    return None, 'h5py'
                    
        except Exception as e2:
            print(f"h5py failed: {e2}")
            return None, 'failed'

def extract_radiomics_features(image, method='scipy'):
...
def process_all_images():
    ...
    # Load MATLAB data
    mat_data, method = load_matlab_data()
    if mat_data is None:
    ...
    if 'images' in mat_data:
        ...
        print(f"Could not find images in MATLAB file. Available keys: {list(mat_data.keys())}")
        return False
    ...
    batch_size = min(100, len(df))  # Process in batches of 100
    ...
    for idx in range(min(len(images), len(df))):
        try:
            ...
            real_features = extract_radiomics_features(image, method)
...
def main():
    """Main function"""
    print("🔬 Extracting REAL Radiomics Features (HDF5 Compatible)")
    print("=" * 60)
