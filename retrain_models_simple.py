#!/usr/bin/env python3
"""
Simple retraining with REAL radiomics features (no parallel processing)
"""

import os
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
from pathlib import Path
from sklearn.model_selection import train_test_split
from sklearn.preprocessing import StandardScaler, LabelEncoder
from sklearn.ensemble import RandomForestClassifier, GradientBoostingClassifier
from sklearn.metrics import (accuracy_score, precision_score, recall_score, f1_score, 
                           roc_auc_score, confusion_matrix, classification_report)
from sklearn.pipeline import Pipeline
import joblib
import warnings
warnings.filterwarnings('ignore')

# Set random seeds for reproducibility
np.random.seed(42)

def load_real_features_data():
    """Load dataset with real radiomics features"""
    print("Loading dataset with REAL radiomics features...")
    
    dataset_path = Path("c:/Users/Lenovo/Desktop/thesis_project/final_ultrasound_dataset_REAL_features.csv")
    if not dataset_path.exists():
        print(f"Real features dataset not found: {dataset_path}")
        return None, None, None
    
    df = pd.read_csv(dataset_path)
    print(f"Loaded dataset: {df.shape}")
    
    # Remove rows with Unknown severity
    df = df[df['severity_label'] != 'Unknown'].copy()
    print(f"After removing Unknown severity: {df.shape}")
    
    # Handle missing values in features
    feature_cols = [col for col in df.columns if col.startswith('feature_') or col in 
                  ['mean_intensity', 'std_intensity', 'min_intensity', 'max_intensity', 
                   'median_intensity', 'q25_intensity', 'q75_intensity', 'skewness', 
                   'kurtosis', 'entropy', 'glcm_contrast', 'glcm_dissimilarity',
                   'glcm_homogeneity', 'glcm_energy', 'glcm_correlation', 'glcm_asm',
                   'area', 'perimeter', 'circularity', 'aspect_ratio', 'extent', 
                   'solidity', 'equivalent_diameter', 'gradient_mean', 'gradient_std',
                   'gradient_max', 'gradient_energy']]
    
    # Fill missing values with median
    for col in feature_cols:
        if col in df.columns:
            df[col] = df[col].fillna(df[col].median())
    
    print(f"Missing values after cleaning: {df[feature_cols].isnull().sum().sum()}")
    
    return df, feature_cols

def create_clinical_feature_mapping():
    """Create mapping of features to clinical meaning"""
    clinical_mapping = {
        'feature_1': 'Mean Intensity - Average echogenicity (fat vs muscle)',
        'feature_2': 'Standard Deviation - Intensity variation',
        'feature_3': 'Minimum Intensity - Darkest tissue regions',
        'feature_4': 'Maximum Intensity - Brightest tissue regions',
        'feature_5': 'Median Intensity - Central echogenicity',
        'feature_6': '25th Percentile - Lower intensity range',
        'feature_7': '75th Percentile - Upper intensity range',
        'feature_8': 'Variance - Intensity spread',
        'feature_9': 'Skewness - Asymmetry of intensity distribution',
        'feature_10': 'Kurtosis - Tailedness of intensity distribution',
        'feature_11': 'Entropy - Tissue organization/disorganization',
        'feature_12': 'GLCM Contrast - Muscle texture variation',
        'feature_13': 'GLCM Dissimilarity - Local intensity differences',
        'feature_14': 'GLCM Homogeneity - Tissue uniformity',
        'feature_15': 'GLCM Energy - Texture uniformity',
        'feature_16': 'GLCM Correlation - Spatial dependency',
        'feature_17': 'GLCM ASM - Angular second moment',
        'feature_18': 'Area - Muscle cross-sectional area',
        'feature_19': 'Perimeter - Muscle boundary length',
        'feature_20': 'Circularity - Muscle shape compactness',
        'feature_21': 'Aspect Ratio - Muscle elongation',
        'feature_22': 'Extent - Muscle space utilization',
        'feature_23': 'Solidity - Muscle convexity',
        'feature_24': 'Equivalent Diameter - Circle equivalent size',
        'feature_25': 'Gradient Mean - Average edge strength',
        'feature_26': 'Gradient Std - Edge variation',
        'feature_27': 'Gradient Max - Strongest edge intensity'
    }
    
    return clinical_mapping

def train_simple_models(X, y_disease, y_severity, feature_names):
    """Train models with real radiomics features (simple version)"""
    print("\n=== Training Models with REAL Radiomics Features ===")
    
    # Disease classification
    print("\n--- Disease Classification ---")
    label_encoder = LabelEncoder()
    y_disease_encoded = label_encoder.fit_transform(y_disease)
    disease_classes = label_encoder.classes_
    
    # Create train/test split
    X_train, X_test, y_train, y_test = train_test_split(
        X, y_disease_encoded, test_size=0.2, stratify=y_disease_encoded, random_state=42
    )
    
    print(f"Train set: {X_train.shape[0]} samples")
    print(f"Test set: {X_test.shape[0]} samples")
    
    disease_models = {}
    
    # Random Forest
    print("\nTraining Random Forest...")
    rf_model = RandomForestClassifier(n_estimators=100, random_state=42, class_weight='balanced')
    rf_pipeline = Pipeline([('scaler', StandardScaler()), ('classifier', rf_model)])
    rf_pipeline.fit(X_train, y_train)
    
    y_pred = rf_pipeline.predict(X_test)
    y_pred_proba = rf_pipeline.predict_proba(X_test)
    
    accuracy = accuracy_score(y_test, y_pred)
    precision = precision_score(y_test, y_pred, average='weighted', zero_division=0)
    recall = recall_score(y_test, y_pred, average='weighted', zero_division=0)
    f1 = f1_score(y_test, y_pred, average='weighted', zero_division=0)
    auc = roc_auc_score(y_test, y_pred_proba[:, 1])
    
    disease_models['Random Forest'] = {
        'accuracy': accuracy, 'precision': precision, 'recall': recall, 'f1': f1, 'auc': auc,
        'pipeline': rf_pipeline, 'confusion_matrix': confusion_matrix(y_test, y_pred)
    }
    
    print(f"Random Forest: Acc={accuracy:.4f}, Prec={precision:.4f}, Rec={recall:.4f}, F1={f1:.4f}, AUC={auc:.4f}")
    
    # Gradient Boosting
    print("\nTraining Gradient Boosting...")
    gb_model = GradientBoostingClassifier(random_state=42)
    gb_pipeline = Pipeline([('scaler', StandardScaler()), ('classifier', gb_model)])
    gb_pipeline.fit(X_train, y_train)
    
    y_pred = gb_pipeline.predict(X_test)
    y_pred_proba = gb_pipeline.predict_proba(X_test)
    
    accuracy = accuracy_score(y_test, y_pred)
    precision = precision_score(y_test, y_pred, average='weighted', zero_division=0)
    recall = recall_score(y_test, y_pred, average='weighted', zero_division=0)
    f1 = f1_score(y_test, y_pred, average='weighted', zero_division=0)
    auc = roc_auc_score(y_test, y_pred_proba[:, 1])
    
    disease_models['Gradient Boosting'] = {
        'accuracy': accuracy, 'precision': precision, 'recall': recall, 'f1': f1, 'auc': auc,
        'pipeline': gb_pipeline, 'confusion_matrix': confusion_matrix(y_test, y_pred)
    }
    
    print(f"Gradient Boosting: Acc={accuracy:.4f}, Prec={precision:.4f}, Rec={recall:.4f}, F1={f1:.4f}, AUC={auc:.4f}")
    
    # Severity classification
    print("\n--- Severity Classification ---")
    severity_encoder = LabelEncoder()
    y_severity_encoded = severity_encoder.fit_transform(y_severity)
    severity_classes = severity_encoder.classes_
    
    # Create train/test split for severity
    X_train_sev, X_test_sev, y_train_sev, y_test_sev = train_test_split(
        X, y_severity_encoded, test_size=0.2, stratify=y_severity_encoded, random_state=42
    )
    
    severity_models = {}
    
    # Random Forest for severity
    print("\nTraining Random Forest for severity...")
    rf_sev_model = RandomForestClassifier(n_estimators=100, random_state=42, class_weight='balanced')
    rf_sev_pipeline = Pipeline([('scaler', StandardScaler()), ('classifier', rf_sev_model)])
    rf_sev_pipeline.fit(X_train_sev, y_train_sev)
    
    y_pred_sev = rf_sev_pipeline.predict(X_test_sev)
    y_pred_proba_sev = rf_sev_pipeline.predict_proba(X_test_sev)[:, 1]
    
    accuracy = accuracy_score(y_test_sev, y_pred_sev)
    precision = precision_score(y_test_sev, y_pred_sev, zero_division=0)
    recall = recall_score(y_test_sev, y_pred_sev, zero_division=0)
    f1 = f1_score(y_test_sev, y_pred_sev, zero_division=0)
    auc = roc_auc_score(y_test_sev, y_pred_proba_sev)
    
    severity_models['Random Forest'] = {
        'accuracy': accuracy, 'precision': precision, 'recall': recall, 'f1': f1, 'auc': auc,
        'pipeline': rf_sev_pipeline, 'confusion_matrix': confusion_matrix(y_test_sev, y_pred_sev)
    }
    
    print(f"Random Forest: Acc={accuracy:.4f}, Prec={precision:.4f}, Rec={recall:.4f}, F1={f1:.4f}, AUC={auc:.4f}")
    
    # Gradient Boosting for severity
    print("\nTraining Gradient Boosting for severity...")
    gb_sev_model = GradientBoostingClassifier(random_state=42)
    gb_sev_pipeline = Pipeline([('scaler', StandardScaler()), ('classifier', gb_sev_model)])
    gb_sev_pipeline.fit(X_train_sev, y_train_sev)
    
    y_pred_sev = gb_sev_pipeline.predict(X_test_sev)
    y_pred_proba_sev = gb_sev_pipeline.predict_proba(X_test_sev)[:, 1]
    
    accuracy = accuracy_score(y_test_sev, y_pred_sev)
    precision = precision_score(y_test_sev, y_pred_sev, zero_division=0)
    recall = recall_score(y_test_sev, y_pred_sev, zero_division=0)
    f1 = f1_score(y_test_sev, y_pred_sev, zero_division=0)
    auc = roc_auc_score(y_test_sev, y_pred_proba_sev)
    
    severity_models['Gradient Boosting'] = {
        'accuracy': accuracy, 'precision': precision, 'recall': recall, 'f1': f1, 'auc': auc,
        'pipeline': gb_sev_pipeline, 'confusion_matrix': confusion_matrix(y_test_sev, y_pred_sev)
    }
    
    print(f"Gradient Boosting: Acc={accuracy:.4f}, Prec={precision:.4f}, Rec={recall:.4f}, F1={f1:.4f}, AUC={auc:.4f}")
    
    return disease_models, severity_models, disease_classes, severity_classes

def analyze_feature_importance(model, feature_names, clinical_mapping, task_name):
    """Analyze feature importance for clinical interpretation"""
    print(f"\n=== Feature Importance Analysis for {task_name} ===")
    
    try:
        # Get the classifier from pipeline
        if hasattr(model, 'named_steps'):
            classifier = model.named_steps['classifier']
        else:
            classifier = model
        
        # Get feature importance
        if hasattr(classifier, 'feature_importances_'):
            importance = classifier.feature_importances_
        else:
            print("Model doesn't have feature_importances_ attribute")
            return None
        
        # Create importance DataFrame
        importance_df = pd.DataFrame({
            'feature': feature_names,
            'importance': importance
        })
        importance_df['clinical_meaning'] = importance_df['feature'].map(clinical_mapping)
        importance_df = importance_df.sort_values('importance', ascending=False)
        
        print(f"\nTop 10 Most Important Features for {task_name}:")
        for i, row in importance_df.head(10).iterrows():
            print(f"  {row['feature']}: {row['importance']:.4f}")
            print(f"    Clinical: {row['clinical_meaning']}")
        
        return importance_df
        
    except Exception as e:
        print(f"Error in feature importance analysis: {e}")
        return None

def create_plots(disease_models, severity_models, disease_importance, severity_importance):
    """Create comprehensive plots"""
    print("\n=== Creating Plots ===")
    
    output_dir = Path("c:/Users/Lenovo/Desktop/thesis_project/output/real_features_results")
    output_dir.mkdir(exist_ok=True, parents=True)
    
    # Performance comparison
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))
    
    # Disease performance
    disease_models_list = list(disease_models.keys())
    disease_acc = [disease_models[name]['accuracy'] for name in disease_models_list]
    disease_f1 = [disease_models[name]['f1'] for name in disease_models_list]
    disease_auc = [disease_models[name]['auc'] for name in disease_models_list]
    
    x = np.arange(len(disease_models_list))
    width = 0.25
    
    ax1.bar(x - width, disease_acc, width, label='Accuracy', alpha=0.8)
    ax1.bar(x, disease_f1, width, label='F1-Score', alpha=0.8)
    ax1.bar(x + width, disease_auc, width, label='AUC', alpha=0.8)
    ax1.set_xlabel('Models')
    ax1.set_ylabel('Score')
    ax1.set_title('Disease Classification Performance (REAL Features)')
    ax1.set_xticks(x + width/2, disease_models_list, rotation=45)
    ax1.legend()
    ax1.grid(True, alpha=0.3)
    
    # Severity performance
    severity_models_list = list(severity_models.keys())
    severity_acc = [severity_models[name]['accuracy'] for name in severity_models_list]
    severity_f1 = [severity_models[name]['f1'] for name in severity_models_list]
    severity_auc = [severity_models[name]['auc'] for name in severity_models_list]
    
    ax2.bar(x - width, severity_acc, width, label='Accuracy', alpha=0.8)
    ax2.bar(x, severity_f1, width, label='F1-Score', alpha=0.8)
    ax2.bar(x + width, severity_auc, width, label='AUC', alpha=0.8)
    ax2.set_xlabel('Models')
    ax2.set_ylabel('Score')
    ax2.set_title('Severity Classification Performance (REAL Features)')
    ax2.set_xticks(x + width/2, severity_models_list, rotation=45)
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(output_dir / 'real_features_performance.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    # Feature importance plots
    if disease_importance is not None:
        plt.figure(figsize=(12, 8))
        top_features = disease_importance.head(10)
        plt.barh(range(len(top_features)), top_features['importance'])
        plt.yticks(range(len(top_features)), [f"{row['feature'][:20]}..." if len(row['feature']) > 20 else row['feature'] for _, row in top_features.iterrows()])
        plt.xlabel('Feature Importance')
        plt.title('Top 10 Important Features - Disease Classification (REAL Features)')
        plt.gca().invert_yaxis()
        plt.tight_layout()
        plt.savefig(output_dir / 'disease_feature_importance.png', dpi=300, bbox_inches='tight')
        plt.close()
    
    if severity_importance is not None:
        plt.figure(figsize=(12, 8))
        top_features = severity_importance.head(10)
        plt.barh(range(len(top_features)), top_features['importance'])
        plt.yticks(range(len(top_features)), [f"{row['feature'][:20]}..." if len(row['feature']) > 20 else row['feature'] for _, row in top_features.iterrows()])
        plt.xlabel('Feature Importance')
        plt.title('Top 10 Important Features - Severity Classification (REAL Features)')
        plt.gca().invert_yaxis()
        plt.tight_layout()
        plt.savefig(output_dir / 'severity_feature_importance.png', dpi=300, bbox_inches='tight')
        plt.close()
    
    print("Plots saved successfully")

def save_all_results(disease_models, severity_models, disease_classes, severity_classes, 
                    clinical_mapping, feature_names):
    """Save all results"""
    print("\n=== Saving Results ===")
    
    output_dir = Path("c:/Users/Lenovo/Desktop/thesis_project/output/real_features_results")
    output_dir.mkdir(exist_ok=True, parents=True)
    
    # Save test results
    test_data = []
    for name, result in disease_models.items():
        test_data.append({
            'Model': name,
            'Task': 'Disease_Classification',
            'Accuracy': result['accuracy'],
            'Precision': result['precision'],
            'Recall': result['recall'],
            'F1_Score': result['f1'],
            'AUC': result['auc']
        })
    
    for name, result in severity_models.items():
        test_data.append({
            'Model': name,
            'Task': 'Severity_Classification',
            'Accuracy': result['accuracy'],
            'Precision': result['precision'],
            'Recall': result['recall'],
            'F1_Score': result['f1'],
            'AUC': result['auc']
        })
    
    test_df = pd.DataFrame(test_data)
    test_df.to_csv(output_dir / 'real_features_test_results.csv', index=False)
    
    # Save trained models
    models_dir = output_dir / 'trained_models'
    models_dir.mkdir(exist_ok=True)
    
    for name, result in disease_models.items():
        model_path = models_dir / f'disease_{name.lower().replace(" ", "_")}_real.joblib'
        joblib.dump(result['pipeline'], model_path)
    
    for name, result in severity_models.items():
        model_path = models_dir / f'severity_{name.lower().replace(" ", "_")}_real.joblib'
        joblib.dump(result['pipeline'], model_path)
    
    # Save clinical mapping
    mapping_df = pd.DataFrame.from_dict(clinical_mapping, orient='index', columns=['Clinical_Meaning'])
    mapping_df.to_csv(output_dir / 'clinical_feature_mapping.csv')
    
    # Save classes
    pd.Series(disease_classes).to_csv(output_dir / 'disease_classes.csv', index=False, header=False)
    pd.Series(severity_classes).to_csv(output_dir / 'severity_classes.csv', index=False, header=False)
    
    print(f"Results saved to: {output_dir}")
    
    return output_dir

def main():
    """Main function"""
    print("🏥 Training Models with REAL Radiomics Features (Simple Version)")
    print("=" * 70)
    
    try:
        # Load real features data
        df, feature_cols = load_real_features_data()
        if df is None:
            return
        
        # Create clinical mapping
        clinical_mapping = create_clinical_feature_mapping()
        
        # Prepare features and labels
        X = df[feature_cols].values
        y_disease = df['label'].values
        y_severity = df['severity_label'].values
        
        print(f"\nDataset Summary:")
        print(f"  Samples: {len(X)}")
        print(f"  Features: {len(feature_cols)}")
        print(f"  Disease classes: {np.unique(y_disease)}")
        print(f"  Severity classes: {np.unique(y_severity)}")
        
        # Train models with real features
        disease_models, severity_models, disease_classes, severity_classes = train_simple_models(
            X, y_disease, y_severity, feature_cols)
        
        # Feature importance analysis
        best_disease_model = max(disease_models.keys(), key=lambda k: disease_models[k]['accuracy'])
        best_severity_model = max(severity_models.keys(), key=lambda k: severity_models[k]['accuracy'])
        
        print(f"\nBest Disease Model: {best_disease_model}")
        print(f"Best Severity Model: {best_severity_model}")
        
        disease_importance = analyze_feature_importance(
            disease_models[best_disease_model]['pipeline'], 
            feature_cols, clinical_mapping, "Disease Classification")
        
        severity_importance = analyze_feature_importance(
            severity_models[best_severity_model]['pipeline'], 
            feature_cols, clinical_mapping, "Severity Classification")
        
        # Create plots
        create_plots(disease_models, severity_models, disease_importance, severity_importance)
        
        # Save all results
        output_dir = save_all_results(disease_models, severity_models, disease_classes, severity_classes,
                                     clinical_mapping, feature_cols)
        
        print("\n" + "=" * 70)
        print("✅ Models trained with REAL radiomics features completed!")
        print(f"📁 Results saved to: {output_dir}")
        print(f"🏥 Clinical interpretation available with feature importance analysis")
        
        # Summary
        print(f"\n📊 Summary:")
        print(f"  Best Disease Model: {best_disease_model} ({disease_models[best_disease_model]['accuracy']:.4f} accuracy)")
        print(f"  Best Severity Model: {best_severity_model} ({severity_models[best_severity_model]['accuracy']:.4f} accuracy)")
        print(f"  Features: REAL radiomics from actual ultrasound images")
        print(f"  Clinical Interpretation: Feature importance with clinical meaning")
        
    except Exception as e:
        print(f"Error in training with real features: {e}")
        import traceback
        traceback.print_exc()

if __name__ == "__main__":
    main()
